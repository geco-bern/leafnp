...
mod_lmm_leafn <- lme(
  ...
  # scaling predictors
  as.formula("log(leafN) ~ scale(ALSA) + scale(mav) + scale(elv) + scale(co2) + scale(tmonthmin) + scale(mai) + scale(ndep)"),
...
mod_lmm_leafp <- lme(
  ...
  as.formula("log(leafP) ~ scale(pmonthmin) + scale(AWC_CLASS) + scale(gs_accl) + scale(alpha) + scale(PBR) + scale(co2) + scale(ai) + scale(tmonthmin) + scale(ndep)"),
  # as.formula(paste("log(", target,") ~ ", paste(shortlist, collapse = " + "))),
  random = ~1|SP,
...
mod_lmm_leafnp <- lme(
  ...
  as.formula("log(LeafNP) ~ scale(map) + scale(ndep) + scale(cwdx80) + scale(elv) + scale(TP) + scale(tmonthmin) + scale(mai) + scale(mapgs) + scale(co2)"),
  # as.formula(paste("log(", target,") ~ ", paste(shortlist, collapse = " + "))),
  random = ~1|SP,
...
df_t_n <- tibble(
  pred = str_remove(
    str_remove(
      str_remove(names(summary(mod_lmm_leafn)$coefficients$fixed), "scale\\("),
      "\\)"),
    "\\("),
  coef = summary(mod_lmm_leafn)$coefficients$fixed,
...
df_t_p <- tibble(
  pred = str_remove(
    str_remove(
      str_remove(names(summary(mod_lmm_leafp)$coefficients$fixed), "scale\\("),
      "\\)"),
    "\\("),
  coef = summary(mod_lmm_leafp)$coefficients$fixed,
...
df_t_np <- tibble(
  pred = str_remove(
    str_remove(
      str_remove(names(summary(mod_lmm_leafnp)$coefficients$fixed), "scale\\("),
      "\\)"),
    "\\("),
  coef = summary(mod_lmm_leafnp)$coefficients$fixed,
...
```{r}
ggt1 <- df_t_n %>% 
  mutate(pred = fct_reorder(pred, coef)) %>%
  ggplot(aes(pred, coef)) +
  geom_bar(stat = "identity", fill = "#777055ff") +
  labs(title = "Leaf N", x = "", y = "Coefficient in LMM") +
  coord_flip() +
  theme_classic()
...
ggt2 <- df_t_p %>% 
  ...
  ggplot(aes(pred, coef)) +
  geom_bar(stat = "identity", fill = "#777055ff") +
  labs(title = "Leaf P", x = "", y = "Coefficient in LMM") +
  coord_flip() +
  theme_classic()
...
ggt3 <- df_t_np %>% 
  ...
  ggplot(aes(pred, coef)) +
  geom_bar(stat = "identity", fill = "#777055ff") +
  labs(title = "Leaf N:P", x = "", y = "Coefficient in LMM") +
  coord_flip() +
  theme_classic()
