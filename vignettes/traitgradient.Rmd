...
gg3 <- df %>% 
  ...
  geom_smooth(method = "lm", se = FALSE, size = 0.1, alpha = 0.2, colour = "darkgoldenrod3") +
  geom_abline(intercept=0, slope=1, linetype="dotted") +
  coord_equal() +
  theme_classic() + 
...
cowplot::plot_grid(gg1, gg2, gg3, labels = c("a", "b", "c"), ncol = 3)
ggsave(paste0(here::here(), "/fig/tga_log.pdf"), width = 10, height = 4)
```
...
df_tga <- df %>% 
  ...
  group_by(Species) %>% 
  nest() %>% 
  mutate(linmod_n  = purrr::map(data, ~lm(log_leafN  ~ log_leafN_sitemean,  data = ., x = TRUE)),
...
```{r}
cols <- c("slope_n" = "royalblue3", "slope_p" = "tomato3", "slope_np" = "darkgoldenrod3")
...
df_tga %>% 
  ...
  # geom_histogram(aes(slope_np, ..density..), fill = "tomato3", binwidth = 0.1, alpha = 0.5) +
  geom_density(aes(slope_np, ..density.., color = "slope_np")) +
  scale_color_manual(values = cols, labels = c("N", "P", "N:P")) +
  xlim(-2,3) +
  theme_classic() +
