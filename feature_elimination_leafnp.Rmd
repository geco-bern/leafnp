...
```{r}
filn <- "data/mod_rf_species_caret_leafn.rds"
...
if (file.exists(filn) && !overwrite){
  ...
  df1h <- df %>% 
    recipe(leafN ~ ., data = dplyr::select(df, leafN, all_of(c(shortlist, "Species")))) %>% 
    step_dummy(Species, one_hot = TRUE) %>% 
    prep(training = df, retain = TRUE) %>% 
    juice()
 
  ## create generic formula for the model and define preprocessing steps
  pp <- recipe(leafN ~ ., data = dplyr::select(df, leafN, all_of(c(shortlist, "Species")))) %>%
  ...
  mod_rf_caret_leafn <- train(
    pp,
    data            = dplyr::select(df, leafN, all_of(c(shortlist, "Species"))),
    metric          = "RMSE",
    method          = "ranger",
    tuneGrid        = tune_grid,
    trControl       = traincotrlParams,
...
```{r}
df_fe_summary %>% 
  mutate(highlight = ifelse(pred == "vcmax25", "yes", "no")) %>% 
  ggplot(aes(pred, rsq, fill = highlight)) +
  scale_fill_manual( values = c( "yes"="tomato", "no"="gray50" ), guide = FALSE ) +
  geom_bar(stat = "identity") +
  labs(x = "", y = expression(italic(R)^2)) +
