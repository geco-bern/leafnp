...
```{r eval=FALSE}
# df_sites <- write_csv("data/df_sites_leafnp_20210324.csv")
...
```{r}
df_hwsd <- df_hwsd %>% 
    mutate(data = purrr::map(data, ~dplyr::select(., T_BULK_DENSITY, AWC_CLASS, T_CLAY, T_SILT, T_SAND, T_GRAVEL, T_PH_H2O, T_TEB, T_BS, T_CEC_SOIL, T_CEC_CLAY, T_ECE, T_ESP, T_CACO3, T_OC)))
```
...
```{r}
settings_gsde <- list(varnam = c("PBR", "PHO", "TP", "TK", "TS"), layer = 1:8)
```
...
save(df_gsde, file = "data/df_gsde_20210324.RData")
```
...
```{r}
## data prepared by di
df_soil <- read_csv("~/data/LeafNP_tiandi/Global_total_leaf_N_P_Di/soil_property_extraction_20210323/lobal_leaf_NP_with_soil_property_from_HWSD_WISE_GSDE_23032021.csv")
