...
dat<-read.csv("global_NP_phylogenetic_Jul_29_update.csv") %>% 
     ...
     unnest() %>% dplyr::filter(LeafNP < 200)%>% 
...
```
#variance partition based on lm analysis
```{r}
#leaf N
NM_null <- lm(log(leafN)~SP,data = dat)
NM_full1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                               ...
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                ...
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = dat2)

NM_full2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = dat2)
anova(NM_null)
anova(NM_full1)
anova(NM_full2)
```
...
```{r}
#leaf P
PM_full1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_full2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P
NPM_full1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_full2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on woody species 

```{r}
# for df_wood [wood = (FG != "Herb" & FG != "Grass" & FG != "Fern")]
#leaf N 
NM_wood1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_wood)

NM_wood2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_wood)
anova(NM_wood1)
anova(NM_wood2)
```
...
```{r}
#leaf P
PM_wood1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_wood2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P
NPM_wood1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_wood2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on tree

```{r}
#leaf N 
NM_tree1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_tree)

NM_tree2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_tree)
anova(NM_tree1)
anova(NM_tree2)
```
...
```{r}
#leaf P 
PM_tree1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_tree2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P 
NPM_tree1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_tree2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on conifers

```{r}
#leaf N 
NM_Conifer1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_Conifer)

NM_Conifer2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_Conifer)
anova(NM_Conifer1)
anova(NM_Conifer2)
```
...
```{r}
#leaf P 
PM_Conifer1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_Conifer2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P 
NPM_Conifer1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_Conifer2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on evergreen broadleaf trees
```{r}
#leaf N 
NM_Eb1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_Eb)

NM_Eb2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_Eb)
anova(NM_Eb1)
anova(NM_Eb2)
```
...
```{r}
#leaf P 
PM_Eb1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_Eb2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P 
NPM_Eb1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_Eb2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on deciduous broadleaf trees
```{r}
#leaf N 
NM_Deci1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_Deci)

NM_Deci2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_Deci)
anova(NM_Deci1)
anova(NM_Deci2)
```
...
```{r}
#leaf P 
PM_Deci1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_Deci2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P 
NPM_Deci1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_Deci2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
## explaination of SP and environment on grasses

```{r}
#leaf N 
NM_grass1 <- lm(terms(log(leafN)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans),keep.order = T),data = df_grass)

NM_grass2 <- lm(terms(log(leafN)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
                                T_BULK_DENSITY_trans + T_CEC_CLAY_trans + T_BS_trans + T_CACO3_trans + 
                                T_SAND_trans + T_SILT_trans + T_CLAY_trans + T_OC_trans + T_CEC_SOIL_trans +
                                 T_TEB_trans + T_ESP_trans + T_ECE_trans)+(elv_trans + gti_trans + ndep_trans +                                   co2_trans)+ SP,keep.order = T),data = df_grass)
anova(NM_grass1)
anova(NM_grass2)
```
...
```{r}
#leaf P 
PM_grass1 <- lm(terms(log(leafP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
PM_grass2 <- lm(terms(log(leafP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
```{r}
#leaf N:P 
NPM_grass1 <- lm(terms(log(LeafNP)~SP +(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
NPM_grass2 <- lm(terms(log(LeafNP)~(mat_trans + matgs_trans + tmonthmin_trans + tmonthmax_trans + 
                                ndaysgs_trans + mai_trans + maigs_trans + map_trans + pmonthmin_trans +
                                mapgs_trans + mavgs_trans + mav_trans + alpha_trans + vcmax25_trans +
                                jmax25_trans + gs_accl_trans + aet_trans + ai_trans + cwdx80_trans)+
                               (pH_trans + TP_trans + PBR_trans +  TOTN_trans + TK_trans + ORGC_trans +                                           CNrt_trans + ALSA_trans + AWC_CLASS_trans + T_GRAVEL_trans + 
...
library(MuMIn)#This is the same as Sardan's 2020
r.squaredGLMM(NM1)
r.squaredGLMM(NM2)
r.squaredGLMM(NM3)
#install.packages("performance")
library(performance)#This is new released July 21 2021 with many indicators
...
preds_soil <- c("T_BULK_DENSITY_trans", "AWC_CLASS_trans", "T_CLAY_trans", "T_SILT_trans", "T_SAND_trans",
                ...
                "T_CEC_CLAY_trans", "T_ECE_trans", "T_ESP_trans", "T_CACO3_trans", "T_OC_trans", "ORGC_trans",
                "TOTN_trans", "CNrt_trans", "ALSA_trans", "PBR_trans", "TP_trans", "TK_trans")
  
preds_climate <- c("mat_trans", "matgs_trans", "tmonthmin_trans", "tmonthmax_trans", "ndaysgs_trans",
                   "mai_trans", "maigs_trans", "map_trans", "pmonthmin_trans", "mapgs_trans", "mavgs_trans",
                   "mav_trans", "alpha_trans", "vcmax25_trans", "jmax25_trans", "gs_accl_trans", "aet_trans",
                   "ai_trans", "cwdx80_trans")
